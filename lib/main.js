...
export default {
  ...
  getProgram(linter, configurationPath) {
    let program;
    try {
      const tsconfigPath = path.resolve(path.dirname(configurationPath), TSCONFIG_FILENAME);
      const isFile = fs.statSync(tsconfigPath).isFile();
      if (isFile) {
        program = linter.createProgram(TSCONFIG_FILENAME, path.dirname(configurationPath));
      }
    } catch (err) {
      // no-op
    }
